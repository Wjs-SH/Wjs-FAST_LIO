...
void Preprocess::hesai_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Hesai XT per-point 'timestamp' is ABSOLUTE seconds. FAST-LIO expects a per-point
  // time OFFSET (ms) relative to scan start (curvature), so subtract the first point's
  // stamp. Audit: point[0].timestamp ~= header stamp; per-scan span ~0.0999 s @ 10 Hz.
  double time_base = pl_orig.points[0].timestamp;
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    const auto &sp = pl_orig.points[i];
    if (!std::isfinite(sp.x) || !std::isfinite(sp.y) || !std::isfinite(sp.z))
      continue;  // /lidar_points is_dense=false -> drop invalid returns

    double range = sp.x * sp.x + sp.y * sp.y + sp.z * sp.z;
    if (range < (blind * blind))
      continue;
    // Optional max-range crop (env FASTLIO_MAXRANGE, metres; 0 = off = original behaviour).
    // XT16 spec range is ~40 m; stray returns out to ~98 m blow up the PCL VoxelGrid index
    // (extent/leaf)^3 and force a coarse filter_size_surf/map. Cropping them lets the
    // registration grid go finer, which is what actually sets wall sharpness.
    static const double s_maxr = [](){ const char* e = std::getenv("FASTLIO_MAXRANGE");
                                       return e ? atof(e) : 0.0; }();
    if (s_maxr > 0.0 && range > (s_maxr * s_maxr))
      continue;
    ...
    added_pt.y = sp.y;
    added_pt.z = sp.z;
    added_pt.intensity = sp.intensity;
    added_pt.normal_x = 0;
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.curvature = (sp.timestamp - time_base) * 1000.0;  // sec -> ms
